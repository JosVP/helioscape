...
One note: the waypoint arrays in Prompt 2 are the one thing Cursor can't generate for you — those depend on where you've actually placed nodes on the map. Define those manually once you've laid out the map in the editor, then reference them by slot ID.
