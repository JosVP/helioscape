# Helioscape — Copilot Prompts: Bio-Phase Management + Tech Tree Updates
> Supplement to copilot_prompts_v2.md.
> Before using a prompt from this file in Copilot Chat, attach or read ARCHITECTURE.md, bio-phase-management.md,
> and earth-tech-tree-options.md.
...
earth_automated_food_systems
  ...
  [→ Copilot: prerequisites accept an OR condition — implement as an array with a "mode": "any"
  ...
  [→ Copilot: add a comment explaining that "present_fork" is a new effect type. Unlike a normal
...
earth_ocean_cleanup
  ...
  [→ Copilot: spillover_gate is a new field. TechTreeSystem checks this in can_unlock(): if
...
earth_rewilding
  ...
  [→ Copilot: the "earth_rewilding_done" flag in GameState is checked by BioPhaseSystem to
...
earth_vertical_megacity
  ...
  [→ Copilot: available at the same tier as earth_advanced_renewables — early game. Consolidates
...
earth_arcology_framework
  ...
  [→ Copilot: dense_living bonus stored in GameState.colonist_bonuses["dense_living"] = true.
...
earth_geoengineering_bureau
  ...
  [→ Copilot: geoengineering_active flag is checked by TerraformingSystem to reduce variance
...
earth_neural_digital_integration
  ...
  [→ Copilot: neural_digital_active is checked by CultureEventSystem when the Fermi Silence
...
earth_planetary_coordination_network
  ...
  [→ Copilot: this is the Architect endgame capstone — equivalent in narrative weight to
...
# [→ Copilot: these are set to true by TechTreeSystem when the relevant tech effects fire.
...
"present_fork":
    ...
    # [→ Copilot: add signal tech_fork_presented(planet_id: String, node_id: String) to
...
- sulphur_reducing_bacteria (naturalist, base, both — especially venus early)
  ...
  [→ Copilot: note the O₂ conflict — sulphur reducers die in oxygen-rich environments.
...
- free_living_fixers (naturalist, base, both)
  [→ Copilot: note "fails silently" if no organic matter is present yet — this is not flagged
...
- engineered_nitrogen_factories (architect, moon_t1, both)
  ...
  [→ Copilot: add "package_incompatibility_note": "Placing alongside naturalist organism packages
...
- decomposer_insects (naturalist, base, both)
  ...
  [→ Copilot: incompatible_with here means "risky without prey base" — add "conditional_incompatible":
...
- cold_optimised_pioneer (architect, moon_t1, mars)
  [→ Copilot: this is the "cold-optimised strain" referenced in the Frost-Hardy Pioneer
...
acidic_soil_remediation_package
  ...
  [→ Copilot: add a "near_mandatory" flag: true — BioPhaseSystem uses this to add a
...
hybrid_adaptives
  ...
  [→ Copilot: trigger_organisms here use wildcard ids. BioPhaseSystem must check by organism
  ...
  [→ Copilot: the discovered organism itself has "tag": "neutral" and a note: "resolves_tension: true"
...
first_soil_fauna
  ...
  [→ Copilot: this discovery requires 4 organisms in the composition (3 required + at least
...
atmospheric_fixer
  ...
  [→ Copilot: effects_on_discover is evaluated immediately when the discovery fires in
...
precipitation_enhancement
  ...
  [→ Copilot: BioPhaseSystem checks availability_condition at composition time. If the condition
...
atmospheric_micro_adjustment
  ...
  [→ Copilot: this booster runs a simultaneous 10yr research track. BioPhaseSystem must
...
restored_earth_seedbank
  ...
  ongoing_cost: {}
  "passive": true
  [→ Copilot: passive: true means this booster is applied automatically when conditions are
...
bio_2 — "Atmospheric Oxygen"
  ...
  [→ Copilot: add a note that the Acidic Soil Remediation Package from Moon T2 is flagged
...
[→ Copilot: add all new signals to EventBus.gd as well.]
...
func _requirements_met(planet_id: String, requirements: Array) -> bool:
    ...
    # [→ Copilot: implement each case in a match block. For bio_phase_progress, check
...
func _calculate_stability(planet_id: String, phase_index: int,
    ...
    # [→ Copilot: implement fully. Load all organisms from DataManager. Check each pair in
...
func _calculate_duration(planet_id: String, phase_index: int, composition: Dictionary,
    ...
    # [→ Copilot: note this is the only system in the game with RNG. The random element is added
...
func _calculate_tag(composition: Dictionary, boosters: Array) -> String:
    ...
    # [→ Copilot: load each organism's tag from DataManager. Count naturalist and architect.
...
func _discovery_conditions_met(discovery: Dictionary, planet_id: String, phase: Dictionary) -> bool:
    ...
    # [→ Copilot: implement the wildcard category check for hybrid_adaptives as described
...
[→ Copilot: add a comment explaining why this is a separate system from DysonSystem and
...
[→ Copilot: add these signals to EventBus.gd.]
...
func _apply_component_arrival(component_id: String, planet: String) -> void:
    ...
    # [→ Copilot: add a comment noting that biodome components are handled by the existing
...
func get_queue_display() -> Array:
    ...
    return []  # [→ Copilot: implement — merge component display_name and target_planet
...
[→ Copilot: add a comment explaining the three-panel layout: left = ConditionsPanel (current
...
func _refresh_compatibility_warnings() -> void:
    ...
    # [→ Copilot: load organism data from DataManager for each placed organism. Check all pairs.
...
PROJECTED SCENE
  ...
  [→ Copilot: this image is an art asset that won't exist at code time. Show a placeholder
...
STABILITY METER
  ...
  [→ Copilot: derive each annotation by iterating the composition. Use organism data from
...
ce_bio_collapse_mars
  ...
  [→ Copilot: this is the one case where quoting the GDD directly is appropriate — the exact
